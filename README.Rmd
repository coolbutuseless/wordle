...
* `Wordle` R6 Class is the primary way of finding candidate words. It has the 
  following methods:
    * `$new()` to start a new object to help with a new puzzle. Use the
      `word_file` argument to set the location of a list of words to start 
      with. This defaults to `/usr/share/dict/words`  (which should work
      on macOS and many unix-ish systems).
